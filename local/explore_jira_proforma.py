...
def verify_credentials():
    ...
    if JIRA_TOKEN:
        print(f"  Token : Length={len(JIRA_TOKEN)} | Start={JIRA_TOKEN[:12]} | End={JIRA_TOKEN[-12:]}")
    else:
        print("  Token : None")
    
    urls_to_try = [
        JIRA_BASE_URL,
        "https://reach-initiative.atlassian.net"
    ]
...
def search_recent_issues():
    ...
    try:
        ...
        for issue in issues:
            ...
            has_xlsx = any(a.get("filename", "").lower().endswith((".xlsx", ".xls")) for a in attachments)
            print(f"  - Key: {issue.get('key')} | Summary: {issue.get('fields', {}).get('summary')} | Project: {issue.get('fields', {}).get('project', {}).get('key')} | Has Excel Attachment: {has_xlsx}")
        return issues
    except Exception as e:
...
def dump_proforma_forms(issue_key):
    ...
    try:
        ...
        if resp.status_code in (200, 201):
            ...
            for idx, f in enumerate(forms):
                form_id = f.get("id")
                print(f"\n  Form {idx+1}: ID: {form_id} | Name: {f.get('name')} | Submitted: {f.get('submitted')} | State: {f.get('state', {}).get('status')}")
                ...
                if detail_resp.status_code == 200:
                    ...
                    out_path.write_text(json.dumps(form, indent=2, ensure_ascii=False), encoding="utf-8")
            ...
            print(f"  Cloud Forms API returned {resp.status_code}. Falling back to properties API...")
            
    except Exception as e:
        print(f"  Modern Cloud Forms API failed/unsupported: {e}. Falling back to properties API...")
    ...
    try:
        ...
        if resp.status_code == 404:
            print(f"  No ProForma forms property ('proforma.forms') found on issue {issue_key} (404).")
            return
        resp.raise_for_status()
        data = resp.json()
        ...
        for idx, form in enumerate(forms):
            print(f"  Form {idx+1}: ID: {form.get('id')} | Name: {form.get('name')} | State: {form.get('state', {}).get('status')}")
            questions = form.get("design", {}).get("questions", {})
            answers = form.get("state", {}).get("answers", {})
...
def search_jsm_requests():
    ...
    try:
        ...
        for r in requests_list:
            print(f"  - Key: {r.get('issueKey')} | Summary: {r.get('summary')} | Status: {r.get('currentStatus', {}).get('status')}")
