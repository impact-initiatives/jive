...
def check_idempotency(
    ...
    if report_attachment:
        if latest_dataset:
            ...
            # We must skip validation to prevent infinite loops when the
            # JIVE report attachment itself triggers an issue_updated webhook.
            skip_validation = True

    if payload.force_revalidation:
        logger.info(
            "Idempotency guard bypassed via webhook payload flag",
...
def resolve_dataset(
    ...
    1. Direct Attachment (Highest Priority): Check for any .xlsx/.xls files attached
    directly to the Jira ticket.
    2. IMPACT Repository (Secondary): Parse the ProForma form to extract the IMPACT
     Repository page URL and scrape/download.
    ...
    if dataset_path:
        logger.info(
            "Successfully resolved dataset from Jira attachment",
    ...
    logger.info(
        "No direct attachment found, attempting ProForma form parsing",
    ...
    if issue_id and proforma_answers:
        ...
        if page_url:
            logger.info(
                "IMPACT Repository URL found in ProForma answers",
                extra={"issue_key": issue_key, "page_url": page_url},
            )
            ...
            if excel_url:
                ...
                logger.info(
                    "Downloading scraped Excel file from Repository",
                    extra={"issue_key": issue_key, "url": excel_url},
                )
                success = impact_repo.download_excel(excel_url, output_path)
                if success:
                    logger.info(
                        "Successfully resolved dataset from IMPACT Repository scraping",
                        extra={"issue_key": issue_key, "resolved_filename": filename},
                    )
                    return output_path
            else:
                logger.warning(
                    "Failed to scrape Excel download link from repository page",
                    extra={"issue_key": issue_key, "page_url": page_url},
                )
        else:
            logger.info(
                "No IMPACT Repository URL found in ProForma answers", extra={"issue_key": issue_key}
            )
    else:
        logger.warning(
            "Could not resolve issue ID — skipping ProForma parsing", extra={"issue_key": issue_key}
        )

    # ── 3. Webhook/Fallback Secure Link (Tertiary) ──
    if secure_link:
        logger.info(
            "Attempting fallback secure link resolution",
            extra={"issue_key": issue_key, "secure_link": secure_link},
        )
        dataset_path = jira.download_from_secure_link(secure_link, output_dir)
        if dataset_path:
            logger.info(
                "Successfully resolved dataset from fallback secure link",
                extra={"issue_key": issue_key, "resolved_filename": dataset_path.name},
            )
            return dataset_path
...
def resolve_context(
    ...
    if proforma_answers:
        ...
        for label, val in proforma_answers.items():
            if (
                ("link to the resource" in label.lower() or "repository" in label.lower())
                and val
                ...
                admin_errors=[
                    {
                        ...
                        "message": (
                            ...
                            "a schema format error when reading the results."
                            f"  Please contact support. Error details: {str(fallback_err)}"
                        ),
                    }
                ],
            )
...
def publish_results(
    ...
    if file_size_mb > MAX_JIRA_ATTACHMENT_MB:
        ...
        logger.info(
            "Attempting to upload public JSM attachment",
            extra={"issue_key": payload.issue_key, "size_mb": round(file_size_mb, 2)},
        )
        jsm_success = False
        if payload.project_key:
            jsm_success = jira.upload_public_jsm_attachment(
                payload.issue_key, payload.project_key, excel_report_path
            )
